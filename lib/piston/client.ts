...
export async function executeCode(
  ...
  try {
    console.log('Submitting to Piston:', { language, pistonLang: config.language, version: config.version, has_input: !!input });
    ...
    if (axios.isAxiosError(error)) {
      ...
      // If Piston is unreachable, provide a helpful error
 ...
 */
export async function listRuntimes(): Promise<Array<{ language: string; version: string; aliases: string[] }>> {
  const response = await pistonClient.get('/api/v2/runtimes');

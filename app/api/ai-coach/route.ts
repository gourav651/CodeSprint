  ...
  try {
    ...
    if (!apiKey) {
      return NextResponse.json(
        {
          error:
            "Missing Groq API Key. Please add GROQ_API_KEY to your .env file. You can get a free key from Groq (https://groq.com/).",
        },
        { status: 500 },
      );
    }
    ...
    const isChatFlow = Array.isArray(messages) && messages.length > 0;
    if (!isChatFlow && (!problemTitle || !code || !language)) {
      return NextResponse.json(
        { error: "Missing required fields (problemTitle, code, language)." },
        { status: 400 },
      );
    }
    ...
    const promptContext = `Problem: ${problemTitle || "Unknown"}\nDescription:\n${problemDescription || "No description provided."}${codeBlock}${errorSummary}${runSummary}`;
...
5. When code is provided, identify the exact line or expression that is causing the bug or error whenever possible.
6. If a runtime error exists, call out the specific invalid variable, function, or operation that triggered it.
